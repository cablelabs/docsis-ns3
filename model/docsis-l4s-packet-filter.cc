...
TypeId
DocsisLowLatencyPacketFilter::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::docsis::DocsisLowLatencyPacketFilter")
            .SetParent<PacketFilter>()
            .SetGroupName("Docsis")
            .AddAttribute("ClassifyEct0AsLowLatency",
                          "Classify ECT(0) also as Low Latency",
                          BooleanValue(false),
...
DocsisLowLatencyPacketFilter::GenerateHash32(Ptr<const QueueDiscItem> item, uint32_t perturbation)
{
    ...
    NS_LOG_DEBUG("Generate hash for 5-tuple src: " << src << " dst: " << dst << " proto: "
                                                   << (uint16_t)prot << " srcPort: " << srcPort
                                                   << " dstPort: " << dstPort << " hash: " << hash);
    Uflw h;
    h.source = src;
    h.destination = dst;
    h.protocol = prot;
...
DocsisLowLatencyPacketFilter::DoClassify(Ptr<QueueDiscItem> item) const
{
    ...
    if ((hdr.GetEcn() == Ipv4Header::ECN_ECT1) || (hdr.GetEcn() == Ipv4Header::ECN_CE) ||
