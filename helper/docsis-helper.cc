...
void
DocsisHelper::EnablePcapInternal(std::string prefix,
                                 Ptr<NetDevice> nd,
                                 bool promiscuous,
                                 bool explicitFilename)
{
    ...
    if (!device)
    {
        NS_LOG_INFO("DocsisHelper::EnablePcapInternal(): Device "
                    << device << " not of type ns3::docsis::DocsisNetDevice");
        return;
    }
...
void
DocsisHelper::EnableAsciiInternal(Ptr<OutputStreamWrapper> stream,
                                  std::string prefix,
                                  Ptr<NetDevice> nd,
                                  bool explicitFilename)
{
    ...
    if (!device)
    {
        NS_LOG_INFO("DocsisHelper::EnableAsciiInternal(): Device "
                    << device << " not of type ns3::docsis::DocsisNetDevice");
        return;
    }
    ...
    if (!stream)
    {
        ...
        //
        asciiTraceHelper.HookDefaultReceiveSinkWithoutContext<DocsisNetDevice>(device,
                                                                               "MacRx",
                                                                               theStream);

        asciiTraceHelper.HookDefaultEnqueueSinkWithoutContext<DocsisNetDevice>(device,
                                                                               "MacTx",
                                                                               theStream);
        asciiTraceHelper.HookDefaultDequeueSinkWithoutContext<DocsisNetDevice>(device,
                                                                               "PhyTxBegin",
                                                                               theStream);
        asciiTraceHelper.HookDefaultDropSinkWithoutContext<DocsisNetDevice>(device,
                                                                            "MacTxDrop",
                                                                            theStream);
...
DocsisHelper::Install(Ptr<Node> cmts, Ptr<Node> cm)
{
    ...
    // Annex N.3: for downstream use by the CMTS, qDelayCoupledL should be used
    ...
    dual->SetLoopDelayCallback(MakeCallback(&DocsisNetDevice::GetLoopDelayEstimate, devA));
    ...
    // Annex N.3: for upstream use by the CM, qDelayCoupledV should be used
    ...
    dual->SetLoopDelayCallback(MakeCallback(&DocsisNetDevice::GetLoopDelayEstimate, devB));
    ...
    bool connected [[maybe_unused]] = devB->TraceConnectWithoutContext(
        "LowLatencyGrantState",
        MakeCallback(&CmtsUpstreamScheduler::ReceiveUnusedLGrantUpdate, scheduler));
    NS_ASSERT_MSG(connected, "Couldn't hook trace source");
    connected = devB->TraceConnectWithoutContext(
        "ClassicGrantState",
        MakeCallback(&CmtsUpstreamScheduler::ReceiveUnusedCGrantUpdate, scheduler));
    NS_ASSERT_MSG(connected, "Couldn't hook trace source");
...
NetDeviceContainer
DocsisHelper::Install(Ptr<Node> cmts,
                      Ptr<Node> cm,
                      Ptr<AggregateServiceFlow> upstreamAsf,
...
DocsisHelper::AssignStreams(const NetDeviceContainer& docsisDevices, int64_t stream)
{
    ...
    NS_ABORT_MSG_UNLESS(cmDualQueue && cmtsDualQueue && cmtsScheduler,
                        "Error: Devices not fully instantiated");
...
Ptr<GameClient>
DocsisHelper::AddUpstreamGameSession(Ptr<Node> client,
                                     Ptr<Node> server,
                                     uint16_t serverPort,
                                     Time startTime,
                                     Time stopTime,
...
Ptr<GameClient>
DocsisHelper::AddDownstreamGameSession(Ptr<Node> client,
                                       Ptr<Node> server,
                                       uint16_t serverPort,
                                       Time startTime,
                                       Time stopTime,
...
Ptr<GameClient>
DocsisHelper::AddGameSession(Ptr<Node> client,
                             Ptr<Node> server,
                             uint16_t serverPort,
                             Time iaTime,
                             Time iaStdDev,
                             ...
                             Ipv4Header::DscpType dscpValue) const
{
    NS_LOG_FUNCTION(client << server << serverPort << iaTime.GetSeconds() << iaStdDev.GetSeconds()
                           << pktSize << pktStdDev << startTime.GetSeconds()
                           << stopTime.GetSeconds() << dscpValue);
    ...
    PacketSinkHelper sinkHelper("ns3::UdpSocketFactory",
                                Address(InetSocketAddress(Ipv4Address::GetAny(), serverPort)));
...
void
DocsisHelper::AddDashSession(Ptr<Node> client,
                             Ptr<Node> server,
                             uint16_t serverPort,
                             Time startTime,
                             Time stopTime,
                             std::string tcpSocketFactoryType,
                             uint32_t maxDashBytes) const
{
    NS_LOG_FUNCTION(client << server << serverPort << startTime.GetSeconds()
                           << startTime.GetSeconds() << stopTime.GetSeconds()
    ...
    sourceDash.SetAttribute("ReadingTime",
                            StringValue("ns3::ConstantRandomVariable[Constant=2.485]"));
    ...
    PacketSinkHelper sinkDash(tcpSocketFactoryType,
...
DocsisHelper::AddFtpSession(Ptr<Node> client,
                            Ptr<Node> server,
                            uint16_t serverPort,
                            Time startTime,
                            Time stopTime,
                            std::string tcpSocketFactoryType,
                            std::string fileModel) const
{
    NS_LOG_FUNCTION(client << server << serverPort << startTime.GetSeconds()
                           << startTime.GetSeconds() << stopTime.GetSeconds()
                           << tcpSocketFactoryType << fileModel);
    ...
    else if (fileModel == "empirical")
    {
        ...
        sourceFtp.SetAttribute("RandomFileSize",
                               StringValue("ns3::LogNormalRandomVariable[Mu=14.8|Sigma=2.0]"));
    }
    ...
    //
    PacketSinkHelper sinkFtp(tcpSocketFactoryType,
                             InetSocketAddress(Ipv4Address::GetAny(), serverPort));
...
DocsisHelper::GetEndpointIpv4Address(Ptr<Node> node) const
{
    ...
    NS_ABORT_MSG_UNLESS(ipv4->GetNInterfaces() == 2,
                        "Unexpected number of interfaces: " << ipv4->GetNInterfaces());
    Ipv4Address addr;
    addr = ipv4->GetAddress(0, 0).GetLocal();
    NS_ABORT_MSG_UNLESS(addr == Ipv4Address::GetLoopback(),
                        "Loopback address not found on first interface");
...
DocsisHelper::AddPacketBurstDuration(Ptr<Node> client,
                                     Ptr<Node> server,
                                     uint16_t serverPort,
                                     uint16_t clientPort,
                                     DataRate ethernetRate,
                                     uint16_t frameSize,
                                     Time startTime,
                                     Time duration,
                                     Ipv4Header::DscpType dscpValue,
                                     Ipv4Header::EcnType ecnValue) const
{
    NS_LOG_FUNCTION(this << client << server << serverPort << clientPort << ethernetRate
                         << frameSize << startTime << duration << dscpValue << ecnValue);
    ...
    return AddPacketBurstCount(client,
                               server,
                               serverPort,
                               clientPort,
                               ethernetRate,
...
std::pair<Ptr<UdpClient>, Ptr<UdpServer>>
DocsisHelper::AddPacketBurstCount(Ptr<Node> client,
                                  Ptr<Node> server,
                                  uint16_t serverPort,
                                  uint16_t clientPort,
                                  DataRate ethernetRate,
                                  ...
                                  Ipv4Header::EcnType ecnValue) const
{
    NS_LOG_FUNCTION(this << client << server << serverPort << ethernetRate << frameSize << startTime
                         << frameCount << dscpValue << ecnValue);
...
DocsisHelper::AddDutyCycleUdpStream(Ptr<Node> client,
                                    Ptr<Node> server,
                                    uint16_t serverPort,
                                    uint16_t clientPort,
                                    DataRate rate,
                                    ...
                                    Ipv4Header::EcnType ecnValue) const
{
    NS_LOG_FUNCTION(client << server << serverPort << clientPort << rate << packetSize << onTime
                           << offTime << startTime << stopTime);
    ...
    oss2 << "ns3::ConstantRandomVariable[Constant=" << std::fixed << std::setprecision(9)
         << offTime.GetSeconds() << "]";
...
Ptr<FileTransferApplication>
DocsisHelper::AddFileTransfer(Ptr<Node> client,
                              Ptr<Node> server,
                              uint16_t serverPort,
                              Time startTime,
                              Time stopTime,
                              ...
                              Ipv4Header::DscpType dscpValue) const
{
    NS_LOG_FUNCTION(client << server << serverPort << startTime << stopTime << fileSize
                           << useReadingTime << (uint16_t)dscpValue);
    // DSCP configuration
    Ipv4Address serverAddr = GetEndpointIpv4Address(server);
    InetSocketAddress serverSocketAddr(serverAddr, serverPort);
    uint16_t tos = dscpValue << 2;
    ...
    Ptr<FileTransferApplication> ftApplication =
        sourceApps.Get(0)->GetObject<FileTransferApplication>();
...
void
DocsisHelper::PrintFlowSummaries(std::ostream& ostr,
                                 ...
                                 Ptr<FlowMonitor> flowMonitor)
{
         ...
         ++i)
    {
        ...
        ostr << "Flow " << protoStream.str() << " (" << t.sourceAddress << ":" << t.sourcePort
        ...
        if (i->second.rxPackets > 0)
        {
            ...
            ostr << "  Rx throughput: " << i->second.rxBytes * 8.0 / rxDuration / 1000 / 1000
                 << " Mbps\n";
            ostr << "  Mean delay:  "
...
DocsisHelper::PrintConfiguration(std::ostream& ostr, const NetDeviceContainer& c) const
{
    ...
    ostr << "FreeCapacityMean: Average upstream free capacity (bits/sec) = "
         << rVal.Get().GetBitRate() << std::endl;
    scheduler->GetAttribute("FreeCapacityVariation", dVal);
    ostr << "FreeCapacityVariation: Bound (percent) on the variation of upstream free capacity = "
         << dVal.Get() << std::endl;
    ...
    ostr << "UsSpectralEfficiency: Upstream spectral efficiency (bps/Hz) = " << dVal.Get()
         << std::endl;
    ...
    ostr << "DsSpectralEfficiency: Downstream spectral efficiency (bps/Hz) = " << dVal.Get()
         << std::endl;
    ...
    ostr << "CmtsDsPipelineFactor: downstream pipeline factor at CMTS = " << uVal.Get()
         << std::endl;
    ...
    ostr << "FrameDuration: Frame duration = " << std::fixed << std::setprecision(3)
         << tVal.Get().As(Time::MS) << std::endl;
    ...
    ostr << "CmMapProcTime: CM MAP processing time = " << std::fixed << std::setprecision(3)
         << tVal.Get().As(Time::MS) << std::endl;
    ...
    ostr << "ActualMapInterval: actual MAP interval = " << std::fixed << std::setprecision(3)
         << tVal.Get().As(Time::MS) << std::endl;
    ostr << std::endl;
    ...
    ostr << "UsGrantsPerSecond: average # of grants scheduled on each US channel = " << dVal.Get()
         << std::endl;
    ...
    ostr << "AvgMapOhPerSymbol: average MAP overhead per symbol (bytes) = " << dVal.Get()
         << std::endl;
