...
DocsisLldTestCase::SetupFourNodeTopology(uint32_t numServiceFlows)
{
    ...
    sf->m_targetBuffer =
        static_cast<uint32_t>(m_upstreamRate.GetBitRate() * 0.25 / 8); // 250ms at MSR
    ...
    sf->m_targetBuffer =
        static_cast<uint32_t>(m_downstreamRate.GetBitRate() * 0.25 / 8); // 250ms at MSR
    if (numServiceFlows == 2)
    {
        Ptr<ServiceFlow> sf2 = CreateObject<ServiceFlow>(LOW_LATENCY_SFID);
    ...
    NetDeviceContainer n2n1Device =
        docsis.Install(m_cmtsNode, m_cmNode, upstreamAsf, downstreamAsf);
    ...
    Ptr<Ipv4Interface> iface0 =
        ifacePair0.first->GetObject<Ipv4L3Protocol>()->GetInterface(ifacePair0.second);
...
DocsisLldTestCase::CheckUpstreamLQueueSize(uint32_t expected)
{
    ...
    NS_TEST_ASSERT_MSG_NE(qdisc, nullptr, "DualQ queue disc not found");
    NS_TEST_ASSERT_MSG_EQ(qdisc->GetLowLatencyQueueSize(),
                          expected,
                          "Expected L-queue length test failure");
}
...
DocsisLldTestCase::CheckUpstreamCQueueSize(uint32_t expected)
{
    Ptr<DualQueueCoupledAqm> qdisc = m_upstream->GetQueue();
    NS_TEST_ASSERT_MSG_NE(qdisc, nullptr, "DualQ queue disc not found");
    NS_TEST_ASSERT_MSG_EQ(qdisc->GetClassicQueueSize(),
                          expected,
                          "Expected C-queue length test failure");
}
...
class DocsisLldSingleUpstreamPacketTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisLldSingleUpstreamPacketTest() override;
...
{
    NS_LOG_DEBUG("Tx callback of size " << p->GetSize() << " at "
...
{
    NS_LOG_DEBUG("MacRxEnd callback of size " << p->GetSize() << " at "
                                              << Simulator::Now().GetMicroSeconds());
    m_lastRxTime = Simulator::Now();
}

void
DocsisLldSingleUpstreamPacketTest::DoSetup()
{
...
void
DocsisLldSingleUpstreamPacketTest::DoRun()
{
    NS_LOG_DEBUG("DocsisLldSingleUpstreamPacketTest::DoRun ()");

    bool connected;
    // Capture trace of Tx events
    connected = m_upstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLldSingleUpstreamPacketTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_downstream->TraceConnectWithoutContext(
        "MacPromiscRx",
        MakeCallback(&DocsisLldSingleUpstreamPacketTest::MacRxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    ...
    connected = m_upstream->TraceConnectWithoutContext(
        "Grant",
        MakeCallback(&DocsisLldSingleUpstreamPacketTest::GrantCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_upstream->TraceConnectWithoutContext(
        "GrantUsed",
        MakeCallback(&DocsisLldSingleUpstreamPacketTest::GrantUsedCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_upstream->TraceConnectWithoutContext(
        "GrantUnused",
        MakeCallback(&DocsisLldSingleUpstreamPacketTest::GrantUnusedCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
#endif
    ...
    NS_TEST_ASSERT_MSG_EQ_TOL(m_lastRxTime,
                              expectedTime,
                              MicroSeconds(2),
                              "Did not see packet at expected time");
}

void
DocsisLldSingleUpstreamPacketTest::DoTeardown()
{
    Simulator::Destroy();
}
...
class DocsisLldSingleDownstreamPacketTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisLldSingleDownstreamPacketTest() override;
...
void
DocsisLldSingleDownstreamPacketTest::DoSetup()
{
    SetupFourNodeTopology(1);
...
void
DocsisLldSingleDownstreamPacketTest::DoRun()
{
    ...
    // Capture trace of Tx events
    bool connected = m_downstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLldSingleDownstreamPacketTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_upstream->TraceConnectWithoutContext(
        "MacPromiscRx",
        MakeCallback(&DocsisLldSingleDownstreamPacketTest::MacRxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
...
void
DocsisLldSingleDownstreamPacketTest::DoTeardown()
{
    Simulator::Destroy();
}
...
class DocsisLldUpstreamPacketTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisLldUpstreamPacketTest() override;
...
uint32_t
DocsisLldUpstreamPacketTest::GetNextPacketSize()
{
...
{
    NS_LOG_DEBUG("Tx drop callback of size " << p->GetSize() << " at "
                                             << Simulator::Now().GetMicroSeconds());
...
void
DocsisLldUpstreamPacketTest::DoSetup()
{
    SetupFourNodeTopology(1);
...
void
DocsisLldUpstreamPacketTest::DoRun()
{
    NS_LOG_DEBUG("DocsisLldUpstreamPacketTest::DoRun ()");

    // Capture trace of Tx events
    bool connected = m_upstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLldUpstreamPacketTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_upstream->TraceConnectWithoutContext(
        "MacTxDrop",
        MakeCallback(&DocsisLldUpstreamPacketTest::DeviceTxDropCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_server->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&DocsisLldUpstreamPacketTest::UdpServerCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
...
void
DocsisLldUpstreamPacketTest::DoTeardown()
{
    Simulator::Destroy();
}
...
class DocsisLldDownstreamPacketTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisLldDownstreamPacketTest() override;
...
void
DocsisLldDownstreamPacketTest::DoSetup()
{
    SetupFourNodeTopology(1);
...
void
DocsisLldDownstreamPacketTest::DoRun()
{
    ...
    bool connected = m_downstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLldDownstreamPacketTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_server->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&DocsisLldDownstreamPacketTest::UdpServerCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
...
void
DocsisLldDownstreamPacketTest::DoTeardown()
{
    Simulator::Destroy();
}
...
class DocsisLldDualQUpstreamPacketTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisLldDualQUpstreamPacketTest() override;
...
void
DocsisLldDualQUpstreamPacketTest::DoSetup()
{
...
void
DocsisLldDualQUpstreamPacketTest::DoRun()
{
    ...
    bool connected = m_upstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLldDualQUpstreamPacketTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_server->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&DocsisLldDualQUpstreamPacketTest::UdpServerCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_efServer->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&DocsisLldDualQUpstreamPacketTest::UdpServerCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
...
void
DocsisLldDualQUpstreamPacketTest::DoTeardown()
{
    Simulator::Destroy();
}
...
class DocsisLldDualQUpstreamTwoPacketReactiveTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisLldDualQUpstreamTwoPacketReactiveTest() override;
...
void
DocsisLldDualQUpstreamTwoPacketReactiveTest::DoSetup()
{
    SetupFourNodeTopology(2);
...
void
DocsisLldDualQUpstreamTwoPacketReactiveTest::DoRun()
{
    ...
    bool connected = m_upstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLldDualQUpstreamTwoPacketReactiveTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_server->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&DocsisLldDualQUpstreamTwoPacketReactiveTest::UdpServerCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_efServer->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&DocsisLldDualQUpstreamTwoPacketReactiveTest::UdpServerCallback, this));
    ...
    NS_LOG_DEBUG("Classic send time: " << nextTime.GetSeconds() << "s; size " << payload
                                       << " bytes");
    m_client->SendPacketAt(nextTime, payload);
    expectedRxCount += payload;
    // Account for padding and headers
...
void
DocsisLldDualQUpstreamTwoPacketReactiveTest::DoTeardown()
{
    Simulator::Destroy();
}
...
class DocsisQueueDiscItemTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisQueueDiscItemTest() override;
...
void
DocsisQueueDiscItemTest::DoSetup()
{
    SetupFourNodeTopology(2);
...
DocsisQueueDiscItemTest::DoRun()
{
    ...
    Simulator::Schedule(MicroSeconds(1002),
                        &DocsisQueueDiscItemTest::CheckUpstreamCQueueSize,
                        this,
                        1528);
    ...
    Simulator::Schedule(MicroSeconds(6002),
                        &DocsisQueueDiscItemTest::CheckUpstreamCQueueSize,
                        this,
                        74);
    ...
    // 1500 IP bytes (1472 + 8 + 20) plus 18 (Ethernet) plus 10 bytes
    // (upstream MAC header size) = 1528 in queue
    Simulator::Schedule(MicroSeconds(11002),
                        &DocsisQueueDiscItemTest::CheckUpstreamLQueueSize,
                        this,
                        1528);
    // Wait a few ms for the data to be sent, and retry with different size
    payload = 12; // UDP payload size in bytes
    ...
    // 40 IP bytes (12 + 8 + 20) plus 24 (Ethernet pads to 64 bytes) plus 10 bytes
    // (upstream MAC header size) = 74 in queue
    Simulator::Schedule(MicroSeconds(16002),
                        &DocsisQueueDiscItemTest::CheckUpstreamLQueueSize,
                        this,
                        74);
...
void
DocsisQueueDiscItemTest::DoTeardown()
{
    Simulator::Destroy();
}
...
class DocsisPacketBurstTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisPacketBurstTest() override;
...
DocsisPacketBurstTest::DoSetup()
{
    ...
    appPair = lldHelper.AddPacketBurstDuration(m_lanNode,
                                               m_wanNode,
                                               udpPort,
                                               udpPort,
                                               DataRate("1Mbps"),
    ...
    udpPort = 10;
    appPair = lldHelper.AddPacketBurstCount(m_lanNode,
                                            m_wanNode,
                                            udpPort,
                                            udpPort,
                                            DataRate("1Mbps"),
...
void
DocsisPacketBurstTest::DoTeardown()
{
    Simulator::Destroy();
}
...
class DocsisTimeToMinislotsTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisTimeToMinislotsTest() override
    {
    }
...
DocsisTimeToMinislotsTest::DocsisTimeToMinislotsTest()
    : DocsisLldTestCase("DocsisTimeToMinislotsTest")
{
}

void
DocsisTimeToMinislotsTest::DoSetup()
{
...
void
DocsisTimeToMinislotsTest::DoRun()
{
    ...
    // must be updated
    NS_TEST_ASSERT_MSG_EQ(m_upstream->TimeToMinislots(Seconds(0)),
                          0,
                          "Incorrect conversion for time 0");
    // Times less than the first frame duration should return zero minislots
    NS_TEST_ASSERT_MSG_EQ(m_upstream->TimeToMinislots(MicroSeconds(134)),
                          0,
                          "Incorrect conversion for time 134us (< 1 frame)");
    // Time equal to first frame duration should return 235 minislots
    NS_TEST_ASSERT_MSG_EQ(m_upstream->TimeToMinislots(MicroSeconds(135)),
                          235,
                          "Incorrect conversion for time 135us (1 frame)");
    // Time slightly greater than first frame duration should return 235 minislots
    NS_TEST_ASSERT_MSG_EQ(m_upstream->TimeToMinislots(MicroSeconds(136)),
                          235,
                          "Incorrect conversion for time 136us (1 frame)");
    ...
    NS_TEST_ASSERT_MSG_EQ(m_upstream->TimeToMinislots(MicroSeconds(2467321560)),
                          4294967160,
                          "Incorrect conversion for large time");
    NS_TEST_ASSERT_MSG_EQ(m_upstream->TimeToMinislots(MicroSeconds(2467321695)),
                          99,
                          "Incorrect roll-over");
}

void
DocsisTimeToMinislotsTest::DoTeardown()
{
    Simulator::Destroy();
}
...
{
  public:
    DocsisLldGgiTest(Time mapInterval,
                     DataRate amsr,
                     DataRate sendRate,
                     DataRate ggr,
                     uint16_t ggi);
    ~DocsisLldGgiTest() override;
...
DocsisLldGgiTest::DocsisLldGgiTest(Time mapInterval,
                                   DataRate amsr,
                                   DataRate sendRate,
                                   DataRate ggr,
                                   uint16_t ggi)
...
{
#ifdef NOTYET
    std::cout << Now().GetSeconds() << " ggr " << report.m_guaranteedGrantRate.GetBitRate()
              << " ggi " << report.m_guaranteedGrantInterval << std::endl;
    std::cout << "mi " << report.m_mapInterval.GetMicroSeconds() << " framespermap "
              << report.m_framesPerMap << std::endl;
    std::cout << "alloc start time " << report.m_message.m_allocStartTime << " msperframe "
              << m_upstream->GetMinislotsPerFrame() << std::endl;
    std::cout << "alloc start time " << report.m_message.m_allocStartTime << " bytesperms "
              << m_upstream->GetMinislotCapacity() << std::endl;
#endif
}

void
DocsisLldGgiTest::DoSetup()
{
...
DocsisLldGgiTest::DoRun()
{
    ...
    bool connected = m_upstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLldGgiTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_upstream->TraceConnectWithoutContext(
        "MacTxDrop",
        MakeCallback(&DocsisLldGgiTest::DeviceTxDropCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_server->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&DocsisLldGgiTest::UdpServerCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_upstream->GetCmtsUpstreamScheduler()->TraceConnectWithoutContext(
        "MapTrace",
        MakeCallback(&DocsisLldGgiTest::MapTraceCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    ...
    if (m_sendRate.GetBitRate() > 0)
    {
        ...
        while (elapsed < Seconds(11))
        {
            ...
            // Account for padding and headers
            m_expectedTxCount += std::max<uint32_t>(46, payload + 28);
            NS_LOG_DEBUG("Send time: " << elapsed.GetSeconds() << "s; size " << payload
                                       << " bytes");
...
class DocsisCheckWraparoundTest : public DocsisLldTestCase
{
  public:
    ...
    ~DocsisCheckWraparoundTest() override;
...
void
DocsisCheckWraparoundTest::DoSetup()
{
    SetupFourNodeTopology(2);
    m_upstream->SetAttribute("MapInterval", TimeValue(MilliSeconds(1)));
...
void
DocsisCheckWraparoundTest::DoRun()
{
    ...
    bool connected = m_upstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisCheckWraparoundTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_upstream->TraceConnectWithoutContext(
        "MacTxDrop",
        MakeCallback(&DocsisCheckWraparoundTest::DeviceTxDropCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_server->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&DocsisCheckWraparoundTest::UdpServerCallback, this));
    connected = m_llServer->TraceConnectWithoutContext(
        "Rx",
        MakeCallback(&DocsisCheckWraparoundTest::UdpServerCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    ...
    while (elapsed < m_stopTime)
    {
        ...
        // Account for padding and headers
        m_expectedTxCount += std::max<uint32_t>(46, payload + 28);
        NS_LOG_DEBUG("Classic send time: " << elapsed.GetSeconds() << "s; size " << payload
                                           << " bytes");
        m_client->SendPacketAt(elapsed, payload);
        m_expectedRxCount += payload;
        // Account for padding and headers
        m_expectedTxCount += std::max<uint32_t>(46, payload + 28);
        NS_LOG_DEBUG("Low latency send time: " << elapsed.GetSeconds() << "s; size " << payload
                                               << " bytes");
...
void
DocsisCheckWraparoundTest::DoTeardown()
{
    Simulator::Destroy();
}
    ...
    : TestSuite("docsis-lld", UNIT)
{
    ...
    AddTestCase(new DocsisLldGgiTest(MilliSeconds(1),
                                     DataRate("50Mb/s"),
                                     DataRate(0),
                                     DataRate("5Mb/s"),
                                     135),
