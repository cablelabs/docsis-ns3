...
class DocsisLinkSingleUpstreamPacketTest : public DocsisLinkTestCase
{
  public:
    ...
    ~DocsisLinkSingleUpstreamPacketTest() override;
...
void
DocsisLinkSingleUpstreamPacketTest::DoSetup()
{
    ...
    sf->m_maxTrafficBurst =
        static_cast<uint32_t>(m_upstreamRate.GetBitRate() * 0.1 / 8); // 100ms at MSR
    ...
    sf->m_maxTrafficBurst =
        static_cast<uint32_t>(m_downstreamRate.GetBitRate() * 0.1 / 8); // 100ms at MSR
...
void
DocsisLinkSingleUpstreamPacketTest::DoRun()
{
    NS_LOG_FUNCTION(this);

    // Ensure that random variables are set as expected
    NS_TEST_ASSERT_MSG_EQ(RngSeedManager::GetRun(),
                          1,
                          "Error:  RngRun number needs to be set to 1");
    ...
    connected = m_upstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLinkSingleUpstreamPacketTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_downstream->TraceConnectWithoutContext(
        "MacPromiscRx",
        MakeCallback(&DocsisLinkSingleUpstreamPacketTest::MacRxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
...
void
DocsisLinkSingleUpstreamPacketTest::DoTeardown()
{
    NS_LOG_FUNCTION(this);
    Simulator::Destroy();
}
...
class DocsisLinkMultipleUpstreamPacketTest : public DocsisLinkTestCase
{
  public:
    ...
    ~DocsisLinkMultipleUpstreamPacketTest() override;
...
void
DocsisLinkMultipleUpstreamPacketTest::DoSetup()
{
    NS_LOG_FUNCTION(this);
    SetupFourNodeTopology();
...
void
DocsisLinkMultipleUpstreamPacketTest::DoRun()
{
    NS_LOG_FUNCTION(this);
    ...
    connected = m_upstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLinkMultipleUpstreamPacketTest::DeviceTxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_downstream->TraceConnectWithoutContext(
        "MacPromiscRx",
        MakeCallback(&DocsisLinkMultipleUpstreamPacketTest::MacRxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
...
void
DocsisLinkMultipleUpstreamPacketTest::DoTeardown()
{
    NS_LOG_FUNCTION(this);
    Simulator::Destroy();
}
...
class DocsisLinkCbrTwoServiceFlowTest : public DocsisLinkTestCase
{
  public:
    ...
    ~DocsisLinkCbrTwoServiceFlowTest() override;
...
DocsisLinkCbrTwoServiceFlowTest::DocsisLinkCbrTwoServiceFlowTest(uint32_t numPackets,
                                                                 uint32_t ggr,
                                                                 uint16_t ggi)
...
void
DocsisLinkCbrTwoServiceFlowTest::DoSetup()
{
    NS_LOG_FUNCTION(this);
    ...
    downstreamAsf->m_maxTrafficBurst =
        static_cast<uint32_t>(m_downstreamRate.GetBitRate() * 0.1 / 8);
...
void
DocsisLinkCbrTwoServiceFlowTest::DoRun()
{
    NS_LOG_FUNCTION(this);
    ...
    connected = m_upstream->TraceConnectWithoutContext(
        "MacTx",
        MakeCallback(&DocsisLinkCbrTwoServiceFlowTest::DeviceTxCallback, this));
    connected = m_upstream->TraceConnectWithoutContext(
        "ClassicGrantReceived",
        MakeCallback(&DocsisLinkCbrTwoServiceFlowTest::CGrantCallback, this));
    connected = m_upstream->TraceConnectWithoutContext(
        "LowLatencyGrantReceived",
        MakeCallback(&DocsisLinkCbrTwoServiceFlowTest::LGrantCallback, this));
    connected = m_upstream->TraceConnectWithoutContext(
        "ClassicRequest",
        MakeCallback(&DocsisLinkCbrTwoServiceFlowTest::CRequestCallback, this));
    connected = m_upstream->TraceConnectWithoutContext(
        "LowLatencyRequest",
        MakeCallback(&DocsisLinkCbrTwoServiceFlowTest::LRequestCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    connected = m_downstream->TraceConnectWithoutContext(
        "MacPromiscRx",
        MakeCallback(&DocsisLinkCbrTwoServiceFlowTest::MacRxCallback, this));
    NS_TEST_ASSERT_MSG_EQ(connected, true, "Could not hook callback");
    ...
    NS_TEST_ASSERT_MSG_EQ(m_txByteCount,
                          2 * m_numPackets * (1000),
                          "Did not see transmitted bytes");
    ...
    NS_TEST_ASSERT_MSG_LT((m_lastRxTime - m_lastTxTime).GetSeconds(),
                          0.010,
                          "Excessive latency observed");

    NS_LOG_DEBUG("TX count " << m_txPacketCount << " RX count " << m_rxPacketCount);
    NS_LOG_DEBUG("Last TX " << m_lastTxTime.GetSeconds() << " RX " << m_lastRxTime.GetSeconds()
                            << " diff (ms) " << (m_lastRxTime - m_lastTxTime).GetSeconds() * 1000);
    NS_LOG_DEBUG("Last C request time " << m_lastCRequestTime.GetSeconds() << " L request time "
                                        << m_lastLRequestTime.GetSeconds());
}

void
DocsisLinkCbrTwoServiceFlowTest::DoTeardown()
{
    NS_LOG_FUNCTION(this);
    Simulator::Destroy();
}
