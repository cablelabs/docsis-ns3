...
DocsisLinkTestCase::GetUpstreamAsf() const
{
    ...
    sf->m_targetBuffer =
        static_cast<uint32_t>(m_upstreamRate.GetBitRate() * 0.25 / 8); // 250 ms at MSR
...
Ptr<AggregateServiceFlow>
DocsisLinkTestCase::GetDownstreamAsf() const
{
    Ptr<AggregateServiceFlow> asf = CreateObject<AggregateServiceFlow>();
    asf->m_asfid = 1;
    ...
    Ptr<ServiceFlow> sf = CreateObject<ServiceFlow>(CLASSIC_SFID);
    sf->m_targetBuffer =
        static_cast<uint32_t>(m_downstreamRate.GetBitRate() * 0.25 / 8); // 250 ms at MSR
    asf->SetClassicServiceFlow(sf);
    Ptr<ServiceFlow> sf2 = CreateObject<ServiceFlow>(LOW_LATENCY_SFID);
    asf->SetLowLatencyServiceFlow(sf2);
    return asf;
...
void
DocsisLinkTestCase::SetupFourNodeTopology()
{
